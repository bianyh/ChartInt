...
## 你的任务
请基于我提供的 ECharts 代码，生成一道关于**“增强图表初始交互性”**的评估题目。你需要完成以下三个步骤，并返回一个 JSON 对象：

1.  **分析与定义编辑任务 (固定为交互维度)**
    * **任务目标**：必须设计一个任务，要求在图表初始化完成后，**自动模拟鼠标悬停在第一个数据项（即“片区1”或索引为0的数据）上的效果**。
    * **效果要求**：
        1.  该数据项需要处于高亮（Highlight）状态。
        2.  该数据项对应的提示框（Tooltip）需要自动显示。
        3.  这种状态主要用于向用户展示图表的交互能力，而无需用户手动操作。
    * **任务描述生成**：请生成一段清晰的、用户视角的文字描述（edit_content）。描述中不要包含具体代码（如 `dispatchAction`），而是描述功能需求。
        * *示例参考*：“请让图表在加载时默认选中第一个片区”、“打开图表时自动显示片区1的详细信息并高亮”等。

2.  **生成参考代码 (Reference HTML)**：
    * 编写一个完整的 HTML 文件，基于原代码内容，实现上述交互需求。
    * **实现方式**：必须在 `setOption` 之后，使用 ECharts 的 `dispatchAction` API 来触发 `highlight` 和 `showTip` 事件。建议包裹在 `setTimeout` 中以确保渲染就绪。
    * **关键要求**：除了增加这个交互逻辑外，图表的原有样式（颜色、布局、数据）必须与原始代码**完全保持一致**。
...
def main():
    ...
    parser.add_argument("--output-dir", default="./data_interaction", help="输出目录")
