...
def process_one(sample_dir: Path, echarts: EchartsToPNG, echarts_with_data: EchartsToPNGWithData, bot: Union[Qwen3Local, LLMClient], out_root: Path):
    ...
    prompt = f"""
## 任务背景
我正在构建一个“图表代码复现与重构”的基准测试。输入是一段 ECharts 配置代码。

## 你的任务
请基于我提供的 ECharts 代码，生成一道“对图表样式编辑”的评估题目。你需要完成以下三个步骤，并返回一个 JSON 对象：

1.  **分析编辑内容**
    请从以下维度中随机选择一个或组合作为编辑任务，以确保数据集的多样性：
        全局与主题维度（如：配色方案、标题配置等）
        布局与空间维度（如：图例布局、组件重排等）
        坐标轴与度量维度（如：轴线样式、标签样式等）
        系列与图形维度（如：图形样式、图形间距等）
        标注与交互维度（如：数值标签、提示注释、标线与标点等）
    * 分析并设计一个合理的图表编辑任务。
    * 该编辑任务表述需要清晰明确，但表述任务的语句不要涉及到代码详细内容，仅从图片层面来描述。例如字号具体大小、颜色编号的详细不要出现在描述中。
    * 尽量进行大幅度的编辑改动。
    
2.  **生成参考代码 (Reference HTML)**：
    * 编写一个完整的 HTML 文件，基于原代码内容，实现编辑后的修改。
    * **关键要求**：除了编辑任务的改变外，图表的所有其他样式必须与原始代码**完全保持一致**。
    - **严格禁止修改**：数据值、图表逻辑（如从饼图变柱状图）
...
## 目标 JSON 结构
{{
    "edit_dimension": "选择的修改维度，多个维度则以‘、’间隔",
    "edit_content": "一段清晰的描述编辑任务的文字。",
...
"""
    ...
    edit_dimension = model_output["edit_dimension"]
    edit_content = model_output['edit_content']
    ...
    edit_content_path = out_dir / "edit_content.txt"
    edit_content_path.write_text(edit_content, encoding="utf-8")

    edit_dimension_path = out_dir / "edit_dimension.txt"
    edit_dimension_path.write_text(edit_dimension, encoding="utf-8")
...
def main():
    ...
    parser.add_argument("--output-dir", default="./data_edit", help="输出目录")
